...
def load_models(modelList:list = [], pretrained:bool= False, EXPERIMENT_NAME: str = None):
    ...
    if not pretrained :
        ...
        for name, model in model_lst.items():
            ...
            try:
                model_lst[name] = ModelCore.load_from_checkpoint(glob.glob(f"{EXPERIMENT_NAME}/{name}/checkpoints/*.ckpt")[0],model = model, num_epochs= settings.NUM_EPOCHS)
            except:
                print(f"❌ resnet50 not available in Experiment: {EXPERIMENT_NAME.split("/")[-1]}")
